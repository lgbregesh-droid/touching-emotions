        ...
        {rows.map((img, i) => (
          <div key={img.id} className="relative group bg-white border border-[#E0D8CC] rounded-lg overflow-hidden">
            <div className="aspect-[4/3] bg-[#EDE6DC] relative">
              ...
              <button onClick={(e) => { e.stopPropagation(); toggle(img); }}
                className={`absolute top-2 right-2 z-20 p-1.5 rounded-full shadow ${img.featured ? "bg-[#BA9B78] text-white" : "bg-white/90 text-gray-600 hover:bg-white"}`}
                title={img.featured ? "מוצגת בדף הבית" : "סמן לדף הבית"}>
                <Star className={`w-4 h-4 ${img.featured ? "fill-white" : ""}`} />
              </button>
              {img.featured && <span className="absolute top-2 left-2 z-20 bg-[#BA9B78] text-white text-[10px] px-2 py-0.5 rounded">דף הבית</span>}
              <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition flex items-end justify-center gap-2 p-2 pointer-events-none">
